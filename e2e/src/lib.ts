...
export type TestConfig = {
    oro_token: string, // on terra
    cw20_ics20: string,
    terra_channel?: string,
    neutron_channel?: string
    new_terra_channel?: string,
    new_neutron_channel?: string
    oro_ibc_denom?: string // on neutron
    oro_tf_denom?: string // on neutron
    oro_tf_ibc_denom?: string // on terra
...
export const ibcTransferOldOro = async (
    signer: Signer,
    config: TestConfig,
    amount: number,
    receiver: string,
    ...
    switch (signer.chain_id) {
        case "localterra-1":
            ...
            const terra_msg = new MsgExecuteContract(signer.address, config.oro_token, inner_msg)
            return simulateAndBroadcast(LCD, signer.signer, signer.chain_id, [terra_msg])
        case "localneutron-1":
            return ibcIcs20Transfer(signer, new Coin(config.oro_ibc_denom!, amount), config.neutron_channel!, receiver, timeout_sec)
        default:
...
export const getCw20Balance = async (config: TestConfig, address: string) => {
    ...
    return LCD.wasm.contractQuery(config.oro_token, msg).then((resp: any) => {
