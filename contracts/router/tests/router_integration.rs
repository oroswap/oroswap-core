...
use oroswap_router::error::ContractError;
use oroswap_test::convert::f64_to_dec;
use oroswap_test::cw_multi_test::{Contract, ContractWrapper, Executor};
use oroswap_test::modules::stargate::StargateApp as App;

use crate::factory_helper::{instantiate_token, mint, mint_native, FactoryHelper, mock_app};
...
fn router_contract() -> Box<dyn Contract<Empty>> {
    Box::new(
        ContractWrapper::new_with_empty(
            oroswap_router::contract::execute,
            oroswap_router::contract::instantiate,
            oroswap_router::contract::query,
        )
        .with_reply_empty(oroswap_router::contract::reply),
    )
}
...
fn router_does_not_enforce_spread_assertion() {
    ...
    let router = app
        .instantiate_contract(
            ...
            owner.clone(),
            &InstantiateMsg {
                oroswap_factory: helper.factory.to_string(),
            },
            &[],
            "router",
            None,
    ...
    let err = app
        .execute_contract(
            ...
            &Cw20ExecuteMsg::Send {
                ...
                amount: 50_000_000000u128.into(),
                msg: to_json_binary(&ExecuteMsg::ExecuteSwapOperations {
                    operations: vec![SwapOperation::OroSwap {
                        offer_asset_info: token_asset_info(token_x.clone()),
                        ask_asset_info: token_asset_info(token_y.clone()),
                        pair_type: PairType::Xyk {},
                    }],
        ...
        .unwrap_err();
    assert_eq!(
        oroswap_pair::error::ContractError::MaxSpreadAssertion {},
        err.downcast().unwrap()
    )
}
...
fn test_swap_route() {
    ...
    let owner = Addr::unchecked("owner");
    let mut helper = FactoryHelper::init(&mut app, &owner);
    let oro = instantiate_token(&mut app, helper.cw20_token_code_id, &owner, "ORO", None);
    ...
    for (a, b, typ, liq) in [
        (&oro, &inj, PairType::Xyk {}, 100_000_000000),
    ...
    // route: oro -> inj, atom -> osmo
    let swap_operations = vec![
        SwapOperation::OroSwap {
            offer_asset_info: AssetInfo::Token {
                contract_addr: oro.clone(),
            },
    ...
    // swap oro for osmo
    // route: oro -> inj, inj -> osmo, osmo -> atom, atom -> osmo
    let swap_operations = vec![
        SwapOperation::OroSwap {
            offer_asset_info: AssetInfo::Token {
                contract_addr: oro.clone(),
    ...
    mint(&mut app, &owner, &oro, swap_amount.u128(), &user).unwrap();
    ...
    let balance_res: BalanceResponse = app
        ...
        .unwrap();
    println!("ORO router balance: {:?}", balance_res.balance);
