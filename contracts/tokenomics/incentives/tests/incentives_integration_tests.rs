...
use oroswap::asset::{native_asset_info, AssetInfo, AssetInfoExt};
use oroswap::incentives::{
...
use oroswap_incentives::error::ContractError;
use oroswap_test::cw_multi_test::Executor;
...
fn test_claim_rewards() {
    ...
    for (token1, token2, lp_token, stakers, alloc_points) in pools.iter_mut() {
        ...
        *lp_token = pair_info.liquidity_token.to_string();
        ...
        for staker in stakers {
            ...
            let oro_before = oro.query_pool(&helper.app.wrap(), &staker_addr).unwrap();
            helper
                .claim_rewards(&staker_addr, vec![pair_info.liquidity_token.to_string()])
                .unwrap_err();
            let oro_after = oro.query_pool(&helper.app.wrap(), &staker_addr).unwrap();
            assert_eq!((oro_after - oro_before).u128(), 0);
    ...
    let oro_before = oro.query_pool(&helper.app.wrap(), &user1).unwrap();
    let err = helper
        .claim_rewards(
            &user1,
            vec![
    ...
    helper
        ...
        .unwrap();
    let oro_after = oro.query_pool(&helper.app.wrap(), &user1).unwrap();
    assert_eq!((oro_after - oro_before).u128(), 2_500000);

    let user2 = TestAddr::new("user2");
    let oro_before = oro.query_pool(&helper.app.wrap(), &user2).unwrap();
    helper
        .claim_rewards(&user2, vec![pools[0].2.to_string(), pools[2].2.to_string()])
        .unwrap();
    let oro_after = oro.query_pool(&helper.app.wrap(), &user2).unwrap();
    assert_eq!((oro_after - oro_before).u128(), 2_500000);
}
...
fn test_incentives() {
    ...
    let err = helper
        .incentivize(&bank, &lp_token, schedule.clone(), &coins(1, "ORO"))
        .unwrap_err();
    assert_eq!(
        err.downcast::<ContractError>().unwrap(),
        ContractError::IncentivizationFeeExpected {
...
fn test_multiple_schedules_same_reward() {
    ...
    // And received ORO rewards
    let oro_reward_balance = oro.query_pool(&helper.app.wrap(), &user).unwrap();
    assert_eq!(oro_reward_balance.u128(), 86400 * 100);
    ...
    let oro_reward_balance = oro.query_pool(&helper.app.wrap(), &user).unwrap();
    assert_eq!(
        oro_reward_balance.u128(),
        u128::from(time_now - time_before_claims) * 100
    );
}

#[test]
fn test_oro_can_bypass_rewards_limit() {
    let oro = native_asset_info("ORO".to_string());
    let mut helper = Helper::new("owner", &oro, false).unwrap();
    let incentivization_fee = helper.incentivization_fee.clone();
...
fn test_oro_external_reward() {
    ...
    // Setup external rewards: 2 equal external ORO rewards that must be summed up
    let bank = TestAddr::new("bank");
    ...
    helper.claim_rewards(&user, vec![lp_token.clone()]).unwrap();
    let oro_reward_balance = oro.query_pool(&helper.app.wrap(), &user).unwrap();
    assert_eq!(oro_reward_balance.u128(), 86400 * (100 + 50));
...
#[test]
/* fn test_oro_protocol_reward_if_denom_changed() {
    let oro = native_asset_info("ibc/old_cw20_oro".to_string());
    let mut helper = Helper::new("owner", &oro, true).unwrap();
    helper
        .app
        .update_block(|block| block.time = Timestamp::from_seconds(EPOCHS_START + EPOCH_LENGTH));
    ...
    let new_oro = native_asset_info("new_oro".to_string());

    // Set new oro token. It replaces old oro token for all active pools
    let msg = ExecuteMsg::UpdateConfig {
        oro_token: Some(new_oro.clone()),
    ...
    // migrate vesting contract with new oro denom; convert all oro to oro2 under the hood
    helper.migrate_vesting(&new_oro.to_string()).unwrap();

    let cycle_end = helper.app.block_info().time.seconds() + 86400 * 7;

    // Iterate one more week by 1 day and claim rewards (should be in new ORO)
    loop {
        let pending = helper.query_pending_rewards(&user, &lp_token);
        let bal_before = helper.snapshot_balances(&user, &pending);
    ...
    let time_now = helper.app.block_info().time.seconds();
    let oro_reward_balance = oro.query_pool(&helper.app.wrap(), &user).unwrap();
    let new_oro_reward_balance = new_oro.query_pool(&helper.app.wrap(), &user).unwrap();
    assert_eq!(
        oro_reward_balance.u128() + new_oro_reward_balance.u128(),
        u128::from(time_now - time_before_claims) * 100
    );
} */
...
fn test_blocked_tokens() {
    ...
    let blk_pair_info = helper
        .create_pair(&[tokens[1].clone(), tokens[2].clone()])
        .unwrap();

    // Check general validation
    let err = helper
        .block_tokens(&guardian, &[oro.clone()])
        .unwrap_err();
    assert_eq!(
        err.root_cause().to_string(),
        format!(
            "Generic error: Blocking ORO token {} is prohibited",
            &oro
        )
    );
    let err = helper
    ...
    // Try to add ORO emissions to the 'blk' pair
    let err = helper
        .setup_pools(vec![
    ...
    assert_eq!(dec256_to_u128_floor(reward_info[0].orphaned), 50 * 1000); // 50 oro * 1000 passed seconds
...
fn test_blocked_pair_types() {
    ...
    assert_eq!(
        dec256_to_u128_floor(reward_info[0].orphaned),
        50 * 1000 // orphaned rewards are not redistributed when blocked pools are deactivated
    );
    let reward_info = helper.query_reward_info(norm_pair2_info.liquidity_token.as_str());
    assert_eq!(
        dec256_to_u128_floor(reward_info[0].orphaned),
...
fn test_update_config() {
    ...
    let msg = ExecuteMsg::UpdateConfig {
        oro_token: Some(AssetInfo::native("new_oro")),
    ...
    assert_eq!(config.oro_token, AssetInfo::native("new_oro"));
