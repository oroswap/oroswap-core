...
use oroswap::asset::{Asset, AssetInfo, AssetInfoExt, PairInfo};
...
use oroswap::pair::StablePoolParams;
use oroswap::vesting::{MigrateMsg, VestingAccount, VestingSchedule, VestingSchedulePoint};
use oroswap::{factory, native_coin_registry, pair, vesting};
...
//     Box::new(
//         ContractWrapper::new_with_empty(
//             oroswap_vesting_131::contract::execute,
//             oroswap_vesting_131::contract::instantiate,
//             oroswap_vesting_131::contract::query,
//         )
//          .with_migrate_empty(oroswap_vesting_131::contract::migrate),
//     )
// }
...
fn generator_contract() -> Box<dyn Contract<Empty>> {
    Box::new(
        ...
        .with_reply_empty(oroswap_incentives::reply::reply),
    )
}
...
impl Helper {
    pub fn new(owner: &str, oro: &AssetInfo, with_old_vesting: bool) -> AnyResult<Self> {
        ...
        // let vesting_code_v131 = app.store_code(vesting_contract_v131());
        let vesting = app
        ...
        let factory = app
            .instantiate_contract(
                ...
                &factory::InstantiateMsg {
                    ...
                    token_code_id,
                    fee_address: Some(TestAddr::new("maker").to_string()),
        ...
        let incentivization_fee = oro
            .with_balance(10_000_000000u128)
            .as_coin()
            .expect("Test suite supports only native oro");
        ...
        let oro_for_vesting = oro.with_balance(u128::MAX).as_coin().unwrap();
        app.init_modules(|router, _, storage| {
            router
                .bank
                .init_balance(storage, &owner, vec![oro_for_vesting.clone()])
        })
        .unwrap();
        app.execute_contract(
            owner.clone(),
    ...
    pub fn create_pair(&mut self, asset_infos: &[AssetInfo]) -> AnyResult<PairInfo> {
        ...
        self.mint_coin(&permissionless, &coin(1000, "uzig"));
