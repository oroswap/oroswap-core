...
use oroswap::asset::{AssetInfo, AssetInfoExt};
use oroswap::incentives::{MAX_PERIODS, MAX_REWARD_TOKENS};
            ...
            Withdraw {
                ...
                if let Ok(total_amount) = helper.query_deposit(lp_token, user) {
                    if total_amount > 0 {
                        ...
                        if let Ok(resp) = helper.unstake(user, lp_token, part) {
                            update_total_rewards(&resp.events, lp_token, &mut rewards);
