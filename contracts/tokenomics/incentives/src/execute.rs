...
pub fn setup_pools(
    ...
    for (active_pool, alloc_points) in &setup_pools {
        ...
        pool_info.set_oro_rewards(&config, *alloc_points);
...
fn update_config(
    ...
    if let Some(oro_token) = oro_token {
        ...
        attrs.push(attr("new_oro_token", oro_token.to_string()));
        config.oro_token = oro_token;

        // Loop through all active pools and update oro asset info
            ...
            let protocol_reward = pool_info
                ...
                .ok_or_else(|| {
                    StdError::generic_err(format!(
                        "Protocol ORO reward not found in active pool {lp_token}",
                    ))
                })?;
            protocol_reward.reward = RewardType::Int(config.oro_token.clone());
            pool_info.save(deps.storage, &lp_token)?;
        }
    }
    ...
    if !add.is_empty() {
        ...
        for token_to_block in &add {
            ...
            if !BLOCKED_TOKENS.has(deps.storage, &asset_info_key) {
                if token_to_block.eq(&config.oro_token) {
                    return Err(ContractError::Std(StdError::generic_err(format!(
                        "Blocking ORO token {token_to_block} is prohibited",
                    ))));
                }
        ...
        if !to_disable.is_empty() {
            ...
            // Update all reward indexes and remove oro rewards from disabled pools
