...
use oroswap::incentives::{Config, IncentivesSchedule, InputSchedule, MAX_ORPHANED_REWARD_LIMIT};
use oroswap::{factory, pair, vesting};
...
/// coin.{lp_minter}.{token_name} where lp_minter is a valid bech32 address on the current chain.
pub fn query_pair_info(deps: Deps, lp_asset: &AssetInfo) -> StdResult<PairInfo> {
    match lp_asset {
        ...
        AssetInfo::NativeToken { denom } => {
            ...
            if denom.starts_with("coin.") && parts.len() >= 3 {
                ...
                Err(StdError::generic_err(format!(
                    "LP token {denom} doesn't follow token factory format: coin.{{lp_minter}}.{{token_name}}",
                )))
            }
        }
    }
    ...
    querier
        .query_wasm_smart::<PairInfo>(
            ...
            &factory::QueryMsg::Pair {
                ...
                pair_type: pair_info.pair_type.clone(),
            },
        )
        .map_err(|_| {
            StdError::generic_err(format!(
