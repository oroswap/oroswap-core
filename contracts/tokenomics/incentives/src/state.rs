...
use oroswap::asset::{Asset, AssetInfo, AssetInfoExt};
use oroswap::common::OwnershipProposal;
use oroswap::incentives::{Config, IncentivesSchedule};
use oroswap::incentives::{PoolInfoResponse, RewardInfo, RewardType};
use oroswap::incentives::{MAX_PAGE_LIMIT, MAX_REWARD_TOKENS};
...
/// Pools which receive ORO emissions
...
impl PoolInfo {
    ...
    /// Set oro per second for this pool according to alloc points and general oro per second value
    pub fn set_oro_rewards(&mut self, config: &Config, alloc_points: Uint128) {
        if let Some(oro_reward_info) = self.rewards.iter_mut().find(|r| !r.reward.is_external()) {
            oro_reward_info.rps = Decimal256::from_ratio(
                config.oro_per_second * alloc_points,
                config.total_alloc_points,
            );
        } else {
            self.rewards.push(RewardInfo {
                reward: RewardType::Int(config.oro_token.clone()),
    ...
    /// Check whether this pools receiving ORO emissions
    ...
    /// This function disables ORO rewards in a specific pool.
    /// We must keep ORO schedule even tho reward per second becomes zero
    pub fn disable_oro_rewards(&mut self) {
        if let Some(oro_reward_info) = self.rewards.iter_mut().find(|r| !r.reward.is_external()) {
            oro_reward_info.rps = Decimal256::zero();
        }
    }
