...
use cosmwasm_std::{
    attr, coin, ensure_eq, from_json, to_json_binary, wasm_execute, Addr, Binary, BankMsg, Coin, CosmosMsg,
...
use oroswap::asset::{
    addr_opt_validate, check_swap_parameters, Asset, AssetInfo, PairInfo,
    MINIMUM_LIQUIDITY_AMOUNT, CoinsExt,
};
use oroswap::common::LP_SUBDENOM;
use oroswap::incentives::ExecuteMsg as IncentiveExecuteMsg;
use oroswap::pair::{
...
use oroswap::{tokenfactory_tracker, U256};

use crate::error::ContractError;
use crate::state::{Config, BALANCES, CONFIG};

/// Contract name that is used for migration.
const CONTRACT_NAME: &str = "oroswap-pair";
...
pub fn instantiate(
    ...
    let pair_config = query_fee_info(&deps.querier, &msg.factory_addr, msg.pair_type.clone())?; // Clone here
...
pub fn swap(
    ...
    let post_offer_reserve = offer_pool.amount + offer_amount;
    let post_ask_reserve = ask_pool.amount
        .checked_sub(return_amount)?
        .checked_sub(maker_fee_amount)?
        .checked_sub(fee_share_amount)?;
    ...
    Ok(Response::new()
        ...
        .add_attributes(vec![
            ...
            // Add current pool reserves after swap in single attribute
...
pub fn migrate(_deps: DepsMut, _env: Env, _msg: Empty) -> Result<Response, ContractError> {
    unimplemented!("No safe path available for migration")
}
