# Oroswap Standard Pair

The standard pool uses the widely known xy=k formula.
...
As an example, let's say the global ratio between two tokens x:y is 10:2 (i.e. 1 x = 0.2 y), but the current ratio between the tokens in an Oroswap pair is 5:2 (1 x = 0.4 y). Let's also say that someone may decide to LP in the x:y Oroswap pool at the current 5:2 ratio. As the Oroswap pool gets arbitraged to the global ratio, the amount of x & y tokens that the LP can withdraw changes because the total amounts of x & y tokens in the pool also change.
...
As an example, let's say someone LPs in a pool and specifies a 1% slippage tolerance. The user LPs 200 UZIG and 1 `ASSET`. With a 1% slippage tolerance, `amountUZIGMin` (the minimum amount of UZIG to LP) should be set to 198 UZIG, and `amountASSETMin` (the minimum amount of `ASSET` to LP) should be set to .99 `ASSET`. This means that, in a worst case scenario, liquidity will be added at a pool rate of 198 `ASSET`/1 UZIG or 202.02 UZIG/1 `ASSET` (200 UZIG + .99 `ASSET`). If the contract cannot add liquidity within these bounds (because the pool ratio changed more than the tolerance), the transaction will revert.
...
Please note that Oroswap has the default value for the spread set to 0.5% and the max allowed spread set to 50%.
