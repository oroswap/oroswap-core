...
use oroswap::asset::{native_asset_info, Asset, AssetInfo, PairInfo, MINIMUM_LIQUIDITY_AMOUNT};
...
use oroswap_pair::error::ContractError;
use oroswap_test::cw_multi_test::{AppBuilder, ContractWrapper, Executor, TOKEN_FACTORY_MODULE};
...
fn instantiate_pair(mut router: &mut TestApp, owner: &Addr) -> Addr {
    ...
    // Change this part to send the pool creation fee:
    let pair = router
        .instantiate_contract(
    ...
    assert_eq!(
        format!("coin.{}.{}", res.contract_addr, LP_SUBDENOM),
        res.liquidity_token
    );
...
fn test_provide_and_withdraw_liquidity() {
    ...
    let mut router = mock_app(
        ...
        vec![
            ...
            Coin {
                denom: "uzig".to_string(),
                amount: Uint128::new(1000), // This matches the pool_creation_fee
            },
        ],
    );
    ...
    assert_eq!(
        ...
        format!("Must send reserve token 'coin.{}.{}'", pair_instance, LP_SUBDENOM)
    );
...
fn test_compatibility_of_tokens_with_different_precision() {
    ...
    app.execute_contract(
        ...
        &[Coin {
            denom: "uzig".to_string(),
            amount: Uint128::new(1000), // This matches the pool_creation_fee we set in the factory config
        }]
    )
    .unwrap();
...
fn test_create_xyk_custom_type() {
    ...
    let mut app = mock_app(
        ...
        vec![
            Coin {
                denom: "uzig".to_string(),
                amount: Uint128::new(2000), // This matches the pool_creation_fee
            },
        ],
    );
    let token_code_id = store_token_code(&mut app);
