...
use cosmwasm_std::{
    ...
    CosmosMsg, Decimal, Decimal256, Deps, DepsMut, Empty, Env, MessageInfo, Reply, Response, StdError,
    StdResult, SubMsg, SubMsgResponse, SubMsgResult, Uint128, WasmMsg, BankMsg,
};
...
use oroswap::cosmwasm_ext::{DecimalToInteger, IntegerToDecimal};
use oroswap::observation::{PrecommitObservation, OBSERVATIONS_SIZE};
use oroswap::pair::{
...
use oroswap_pcl_common::{calc_d, get_xcp};
...
pub fn instantiate(
    ...
    Ok(Response::new()
        ...
        .add_attributes(vec![
            ...
            attr("asset_balances_tracking", if config.track_asset_balances { "enabled" } else { "disabled" }),
...
fn swap(
    ...
    Ok(Response::new().add_messages(messages).add_attributes(vec![
        ...
        attr("reserves", format!("{}:{},{}:{}", 
            pools[offer_ind].info.to_string(),
            (pools[offer_ind].amount + offer_asset_dec.amount).to_uint(offer_asset_prec)?,
            pools[ask_ind].info.to_string(),
            (pools[ask_ind].amount.to_uint(ask_asset_prec)? - return_amount - maker_fee - fee_share_amount)
        )),
    ]))
}
