...
use cosmwasm_std::{
    ...
    StdError, StdResult, SubMsg, SubMsgResponse, SubMsgResult, Uint128, WasmMsg, BankMsg,
};
...
use oroswap::asset::{
    addr_opt_validate, check_swap_parameters, Asset, AssetInfo, DecimalAsset, PairInfo,
    MINIMUM_LIQUIDITY_AMOUNT,
};
use oroswap::common::{claim_ownership, drop_ownership_proposal, propose_new_owner, LP_SUBDENOM};
use oroswap::observation::{query_observation, PrecommitObservation, OBSERVATIONS_SIZE};
use oroswap::pair::{
...
use oroswap::querier::{query_factory_config, query_fee_info, query_native_supply};
use oroswap::token_factory::{tf_burn_msg, tf_create_denom_msg, MsgCreateDenomResponse};
use oroswap::DecimalCheckedOps;
use oroswap_circular_buffer::BufferManager;
use oroswap::asset::{CoinsExt, Decimal256Ext};
use oroswap::cosmwasm_ext::IntegerToDecimal;
...
const CONTRACT_NAME: &str = "oroswap-pair-stable";
...
pub fn instantiate(
    ...
    // Get factory config to check maker fee address
    let factory_config = query_factory_config(&deps.querier, &msg.factory_addr)?;
    let fee_address = factory_config.fee_address.ok_or(ContractError::MakerFeeAddressNotSet {})?;

    // Get pair config to check pool creation fee
    let pair_config = query_fee_info(&deps.querier, &msg.factory_addr, msg.pair_type.clone())?;
    ...
    // Create messages to:
    // 1. Create LP token
    // 2. Forward pool creation fee to maker fee address
    ...
    // Forward pool creation fee to maker fee address
    messages.push(SubMsg::new(CosmosMsg::Bank(BankMsg::Send {
        to_address: fee_address.to_string(),
        amount: vec![coin(pair_config.pool_creation_fee.u128(), "uzig")],
    })));
    ...
    Ok(Response::new()
        ...
        .add_attributes(vec![
            ...
            attr("maker_fee_address", fee_address.to_string()),
            attr("pool_creation_fee", pair_config.pool_creation_fee.to_string()),
        ]))
}
...
pub fn reply(deps: DepsMut, _env: Env, msg: Reply) -> Result<Response, ContractError> {
    match msg {
        Reply {
            ...
            #[cfg(not(feature = "zigchain"))]
            let MsgCreateDenomResponse { new_token_denom } = data.try_into()?;
            #[cfg(feature = "zigchain")]
            let MsgCreateDenomResponse { denom: new_token_denom, .. } = data.try_into()?;
...
/// * **ExecuteMsg::Pause {}** Pauses the pair contract.
/// * **ExecuteMsg::Unpause {}** Unpauses the pair contract.
#[cfg_attr(not(feature = "library"), entry_point)]
pub fn execute(
    deps: DepsMut,
    env: Env,
    ...
    match msg {
        ...
        ExecuteMsg::Pause {} => pause_pair(deps, info),
        ExecuteMsg::Unpause {} => unpause_pair(deps, info),
...
pub fn receive_cw20(
    ...
    check_pair_not_paused(&deps.as_ref())?;
...
/// Check if the pair is paused and return an error if it is.
fn check_pair_not_paused(deps: &Deps) -> Result<(), ContractError> {
    let config = CONFIG.load(deps.storage)?;
    if config.paused {
        return Err(ContractError::PairPaused {});
    }
    Ok(())
}
...
/// Pause the pair (only callable by factory or pair owner).
///
/// ## Executor
/// Only the factory or pair owner can execute this.
pub fn pause_pair(
    deps: DepsMut,
    info: MessageInfo,
) -> Result<Response, ContractError> {
    ...
    // Check authorization (factory or owner)
    let factory_config = query_factory_config(&deps.querier, &config.factory_addr)?;
    if info.sender != config.factory_addr && info.sender != factory_config.owner {
        return Err(ContractError::Unauthorized {});
    }
    ...
    Ok(Response::new()
        .add_attributes(vec![
            ...
            attr("paused_by", info.sender.to_string()),
        ]))
}

/// Unpause the pair (only callable by factory or pair owner).
///
/// ## Executor
/// Only the factory or pair owner can execute this.
pub fn unpause_pair(
    ...
    Ok(Response::new()
        .add_attributes(vec![
            ...
            attr("unpaused_by", info.sender.to_string()),
        ]))
}
