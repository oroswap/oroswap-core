...
use oroswap_test::cw_multi_test::{AppBuilder, AppResponse, Contract, ContractWrapper, Executor};
...
use oroswap_pair_stable::contract::{execute, instantiate, query, reply};

const INIT_BALANCE: u128 = 1_000_000_000000;

pub fn init_native_coins(test_coins: &[TestCoin]) -> Vec<Coin> {
    let mut coins: Vec<Coin> = test_coins
        .iter()
    ...
    // Add uzig tokens for pool creation fee
    coins.push(coin(1000, "uzig"));
