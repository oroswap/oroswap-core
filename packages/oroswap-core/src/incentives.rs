...
/// Token factory token, for example, xORO, with bank hook is ~300k.
...
pub enum ExecuteMsg {
    ...
    /// Set a new amount of ORO to distribute per seconds.
    /// Only the owner can execute this.
    SetTokensPerSecond {
        /// The new amount of ORO to distribute per second
        amount: Uint128,
    },
    ...
    /// Pools which contain these tokens can't be incentivized with ORO rewards.
...
pub enum QueryMsg {
    ...
    #[returns(Vec<(String, Uint128)>)]
    /// Returns the list of all pools receiving oro emissions
    ActivePools {},
}
...
pub struct Config {
    ...
    /// [`AssetInfo`] of the ORO token
    pub oro_token: AssetInfo,
    /// Total amount of ORO rewards per second
    pub oro_per_second: Uint128,
    /// Total allocation points. Must be the sum of all allocation points in all active generators
    pub total_alloc_points: Uint128,
    /// The vesting contract which distributes internal (ORO) rewards
...
pub enum RewardType {
    /// Internal rewards aka ORO emissions don't have next_update_ts field and they are paid out from Vesting contract.
    Int(AssetInfo),
