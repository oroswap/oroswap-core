...
impl Default for OroswapFactoryQuerier {
    fn default() -> Self {
        Self {
            pairs: HashMap::new(),
        }
    ...
    pub fn execute(&self, request: &QueryRequest<Empty>) -> QuerierResult {
        match &request {
            QueryRequest::Wasm(WasmQuery::Smart {
                ...
                FactoryQueryMsg::Pair { asset_infos, pair_type: _ } => {
...
impl WasmMockQuerier {
    ...
    pub fn with_oroswap_pairs(&mut self, pairs: &[(&String, &PairInfo)]) {
        self.query_handler.oroswap_factory_querier = OroswapFactoryQuerier::new(pairs);
    }
