...
/// ```
/// # use oroswap_core::factory::PairType::{Custom, Stable, Xyk};
/// Xyk {};
/// Stable {};
...
// Add this at the module level, before the PairConfig struct
fn default_pool_creation_fee() -> Uint128 {
    Uint128::new(1000)
}
...
pub struct PairConfig {
    ...
    //TODO: add pool creation fee
    #[serde(default = "default_pool_creation_fee")]
...
pub struct InstantiateMsg {
    ...
    pub owner: String,
    /// CW1 whitelist contract code id used to store 3rd party rewards for staking Oroswap LP tokens
    pub whitelist_code_id: u64,
...
pub enum ExecuteMsg {
    ...
    UpdateConfig {
        ...
        /// Contract address where Lp tokens can be auto_staked after someone provides liquidity in an incentivized Oroswap pool
    ...
    PausePair {
        /// The assets for which we pause the pair
        asset_infos: Vec<AssetInfo>,
        /// The pair type to pause
        pair_type: PairType,
    },
    /// Unpause a specific pair by its asset infos
    UnpausePair {
        /// The assets for which we unpause the pair
        asset_infos: Vec<AssetInfo>,
        /// The pair type to unpause
        pair_type: PairType,
    },
    /// Pause pairs in batches (for large numbers of pairs)
    PausePairsBatch {
        /// Number of pairs to process per batch (default: 50, max: 100)
    ...
    AddPauseAuthorities {
        /// Addresses to add as pause authorities
        authorities: Vec<String>,
    },
    /// Remove addresses with pause authority
    RemovePauseAuthorities {
        /// Addresses to remove from pause authorities
        authorities: Vec<String>,
    },
}
...
pub enum QueryMsg {
    ...
    Pair {
        ...
        asset_infos: Vec<AssetInfo>,
        /// The pair type for which we return pair information
        pair_type: PairType,
    },
    ...
    #[returns(bool)]
    IsPairPaused {
        /// The assets for which we check if the pair is paused
        asset_infos: Vec<AssetInfo>,
        /// The pair type to check
        pair_type: PairType,
    },
...
pub struct ConfigResponse {
    ...
    pub fee_address: Option<Addr>,
    /// Address of contract used to auto_stake LP tokens for Oroswap pairs that are incentivized
    pub generator_address: Option<Addr>,
    /// CW1 whitelist contract code id used to store 3rd party rewards for staking Oroswap LP tokens
    pub whitelist_code_id: u64,
    /// The address of the contract that contains the coins and their accuracy
