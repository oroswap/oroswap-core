    ...
    C: CustomQuery,
{
    ...
    Ok(FeeInfo {
        ...
        pool_creation_fee: res.pool_creation_fee, 
    })
}
...
/// * **asset_infos** vector of assets to find the pair for.
/// * **pair_type** type of pair to query for.
...
/// symbolize pairs instantiated in the Oroswap factory
///
/// * **factory_contract** address of the factory contract.
/// * **start_after** optional struct containing both asset infos and pair type to start after.
...
/// Returns a vector that contains items of type [`PairInfo`] for all pairs
/// that contain the specified assets, regardless of pair type.
pub fn query_pairs_by_assets(
    querier: &QuerierWrapper,
    factory_contract: impl Into<String>,
    asset_infos: &[AssetInfo],
) -> StdResult<PairsResponse> {
