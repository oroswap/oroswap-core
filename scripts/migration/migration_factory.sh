...
# Current factory contract address to migrate
export FACTORY_CONTRACT="zig1wn625s4jcmvk0szpl85rj5azkfc6suyvf75q6vrddscjdphtve8sh9354q"
# New code ID to migrate the factory contract to
export NEW_FACTORY_CODE_ID="80"
# Migration message (customize if your contract requires init params)
MIGRATE_MSG='{}'

# ─── Execute migration ───────────────────────────────────────────────────────
echo "Migrating factory contract $FACTORY_CONTRACT to code ID $NEW_FACTORY_CODE_ID..."

# Broadcast migration tx asynchronously and capture tx hash
broadcast_resp=$($BINARY tx wasm migrate "$FACTORY_CONTRACT" "$NEW_FACTORY_CODE_ID" "$MIGRATE_MSG" \
  --from "$KEY_NAME" \
...
echo -e "\nFactory contract migration complete."
